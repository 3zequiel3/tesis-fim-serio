...
|---|---|---|
| `completa` | **2** | US-04, US-13 |
| `parcial` | **28** | US-01, US-02, US-03, US-05, US-06, US-07, US-08, US-10, US-11, US-12, US-14, US-15, US-16, US-17, US-18, US-19, US-20, US-21, US-22, US-23, US-24, US-25, US-26, US-27, US-28, US-29, US-30, US-31 |
...
criterios de aceptación y que 2 los tienen asertados por completo — pero **no** que las 31 historias
estén cubiertas, porque 28 conservan criterios sin verificar y una (US-09) describe una función que
...
| **US-06** Listado de eventos | Tabla paginada 50/página; fila con path, estado, acción, severidad, fecha y proceso causante; orden desc; excluye `superseded` | `test_event_listing_contract.py` (4), `test_c31_backend_event_correctness.py` (1), `eventFilters.test.ts` (2) — [§5.6](#us-06) | parcial | Tres de los cuatro criterios cerrados: el `page_size` por defecto de 50 (con la segunda página verificada), el orden `created_at DESC` con inserción fuera de orden a propósito, y la exclusión de `superseded`. Queda el criterio de contenido de fila, que no sólo carece de test: `EventsTable.tsx` no renderiza severidad, tipo de acción ni contexto de proceso |
...
| **US-25** Bulk approve/reject | Checkbox por fila y "todo en la página"; botones bulk; modal con los primeros 10 paths; elección de acción; `POST /actions/bulk-*`; locking individual C5; `succeeded[]`/`failed[]`; baseline firmado por cada aprobado; `audit_log`; resumen visual; refresco | `test_actions_router.py` (7), `test_actions.py` (6) — [§5.25](#us-25) | parcial | Los endpoints bulk pasaron de no tener ninguna prueba HTTP a tener cuatro, incluido el lote vacío y `absent_confirmation_required` como motivo de fallo por ítem sin cortar el lote. Confirmado por test el contrato real: `items[]` con `{event_id, version, action}`, no `event_ids[]`, y con **acción por ítem**, no una acción única aplicada a todos como pide la historia. Los cinco criterios de interfaz siguen sin test |
...
| El refresh rota en cada uso (C8) | `backend/tests/test_auth.py::test_refresh_valido_rota_token`, `::test_refresh_con_token_revocado_retorna_401` |
| Refresh expirado o revocado → login | `backend/tests/test_auth.py::test_refresh_con_token_revocado_retorna_401`, `::test_refresh_sin_cookie_retorna_401`, `::test_logout_invalida_tambien_el_refresh_token`, `backend/tests/test_c22_auth.py::test_refresh_token_como_bearer_retorna_401_token_type_invalid`, `::test_token_sin_type_claim_retorna_401_token_type_invalid` |
| Renovación transparente | `backend/tests/test_auth.py::test_refresh_concurrente_dentro_de_gracia_reusa_la_sesion_rotada`, `::test_refresh_incluye_objeto_user` (lado servidor; el single-flight del cliente: SIN TEST) |
...
| Elección de acción para el rechazo bulk | `backend/tests/test_actions_router.py::test_bulk_reject_uses_items_contract_with_per_item_action`. **Divergencia**: la acción va por ítem, no una única aplicada a todos los seleccionados como pide la historia |
| `POST /actions/bulk-approve` / `bulk-reject` | `backend/tests/test_actions_router.py::test_bulk_approve_uses_items_contract_and_partitions_results`, `::test_bulk_reject_uses_items_contract_with_per_item_action`, `::test_bulk_endpoints_accept_an_empty_batch` (el lote vacío devuelve la estructura vacía, no un 500), `::test_actions_endpoints_require_authentication[/actions/bulk-approve]`, `::test_actions_endpoints_require_authentication[/actions/bulk-reject]`, `::test_actions_endpoints_reject_non_admin[/actions/bulk-approve]`, `::test_actions_endpoints_reject_non_admin[/actions/bulk-reject]` |
...
| US-25 | Acción única aplicada a todo el lote | El contrato lleva acción **por ítem** |
