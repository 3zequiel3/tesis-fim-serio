...
|-----------|---------|------------|
| `fanotify` requiere `CAP_SYS_ADMIN` | El agente no puede correr dentro de un contenedor Docker estándar | Despliegue nativo como servicio `systemd` en el anfitrión con hardening (`ProtectSystem=strict` con `ReadWritePaths` derivado por drop-in, `NoNewPrivileges`) |
| Remediación automática requiere escritura sobre el filesystem del anfitrión (`CAP_SYS_ADMIN` no exime chequeos DAC) | `auto_restore`/`quarantine` eran físicamente inejecutables con el hardening por defecto | `CAP_DAC_OVERRIDE`/`CAP_FOWNER`/`CAP_CHOWN` sumadas al unit + `ReadWritePaths` derivado de los `watch_paths` (D36/RN-130) |
| Un `watch_path` agregado desde la interfaz (`update_config`) no es remediable hasta regenerar el drop-in en el anfitrión | Ese path queda **monitoreado pero no remediable** — limitación inherente a que systemd resuelve `ReadWritePaths` en el namespace de montaje al arrancar el servicio, no un defecto de implementación | El preflight lo hace visible en la tarjeta del agente (solo-detección) dentro de un heartbeat en vez de descubrirse al fallar una remediación; el operador regenera el drop-in (`sudo bash agent/install.sh`) y hace `systemctl daemon-reload` (D36/RN-130) |
