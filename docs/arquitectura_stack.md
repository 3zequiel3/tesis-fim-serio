...
| D67 | `audit_log.user_id` pasa a nullable mediante migración aditiva (`DROP NOT NULL`, sin backfill). `NULL` identifica una acción originada por el sistema; el actor y los datos viajan en `extra` — para `POST /agents/renew`, el `agent_id` y los seriales saliente y entrante. Se descartó una columna `actor_agent_id` para no partir el concepto de actor en dos campos. Agregada el 2026-09-16. |
| D68 | `Agent.registered_at` (timestamptz no nulo, default `now()`) por migración aditiva. La segunda pasada de `_sweep_offline` (`agents/heartbeat_consumer.py`) agrega la condición `last_heartbeat IS NULL AND registered_at < dead_threshold`, reutilizando `_DEAD_THRESHOLD_S` (300 s). Sin esa condición un agente que nunca latió nunca transiciona, porque `NULL < x` es `NULL` en SQL. Agregada el 2026-09-16. |
| D69 | `detector.out_of_scope_drop` pasa de `warning` a `debug`; `out_of_scope_drops` —ya presente en el payload del heartbeat (`agent/heartbeat.py`)— se persiste en `Agent` por migración aditiva nullable y se muestra en la tarjeta del agente como contador **informativo**, sin tratamiento de anomalía, a diferencia de `discarded_events`. Agregada el 2026-09-17. |
| D70 | La vista del cambio obligatorio de contraseña se sirve en `/change-password` (`frontend/src/App.tsx`, gate en `ProtectedRoute`), ruta vigente desde `3b62b97`; la vista de alertas en fallo terminal se sirve en `/alerts/failed` (`frontend/src/App.tsx`, `GET /alerts/failed`), vigente desde `f8508eb`. Registro retroactivo: el texto de US-01, US-05, US-27 y US-29 se alineó a la implementación en `cc73c2d` sin decisión previa, y RN-102 se reescribió el 2026-09-17. Agregada el 2026-09-17. |
| D71 | Ratifica D2: `enqueue_baseline_update` (`backend/app/modules/actions/streams.py`) usa `event.hash_detected` y nunca relee el filesystem; aprobar el hash releído incorporaría al baseline una modificación posterior no revisada (TOCTOU). El criterio 4 de US-11 se alinea a esta decisión. Agregada el 2026-09-17. |
| D72 | `agent/heartbeat.py` agrega `queue_pressure_high: bool` (`queue_pressure > 0.8`, calculado en el agente) y conserva el float `queue_pressure`. Backend: `Agent.queue_pressure_high: bool \| None` por migración aditiva `021` sin backfill, ingesta tolerante (clave ausente no pisa, no-bool se ignora con log) y exposición en `AgentResponse`. Frontend: el banner W3 de `AgentCard` se deriva sólo del flag, sin respaldo por umbral en el cliente. Agregada el 2026-09-17. |
| D73 | `agent/logging.py` (`configure_logging`) pasa a `wrapper_class=structlog.make_filtering_bound_logger(log_level_int)`, mismo mecanismo que `backend/app/core/logging.py:245`. El nivel llegaba sólo a `logging.basicConfig`, que no filtra la salida de `structlog`, así que con `wrapper_class=structlog.stdlib.BoundLogger` y `logger_factory=structlog.PrintLoggerFactory(sys.stdout)` se imprimía todo nivel — D69/RN-163 bajó `detector.out_of_scope_drop` a `debug` sin efecto real (82 líneas `debug` en 15 s tras el despliegue del 2026-09-17). Agregada el 2026-09-17. |
| D74 | `agent/detector.py` agrega el contador `null_path_drops` (incrementado antes del `log.debug("detector.event_null_path", ...)`, que reemplaza al `log.warning` anterior) y su property pública; `agent/heartbeat.py` publica la clave junto a `out_of_scope_drops` y `event_drops`. Causa raíz investigada en `agent/_fanotify.py:265-337`: `_open_by_handle` devuelve `None` cuando `open_by_handle_at` falla (típicamente `ESTALE`) contra todos los fds de montaje abiertos, en un punto anterior al filtro de scope (que necesita el path para decidir) — a diferencia de `out_of_scope_drop` (D69/RN-163), no hay certeza de que el objeto perdido estuviera fuera de `watch_paths`, así que el contador se documenta como posible brecha de cobertura, no como ruido confirmado. Medido en el host del agente: 135.547 líneas desde el 2026-09-16, 71.833 entre las 13:00 y las 14:44 del 2026-09-17 (~12/s). Sin persistencia en backend ni presentación en frontend (fuera de alcance de esta decisión). Agregada el 2026-09-17. |
| D75 | Amplía D21, cuya enumeración de funciones a desbloquear era cerrada y dejaba afuera el carril de ingesta. `_get_agent_auth` (`backend/app/modules/events/consumer.py:305`), `_ingest` (`:400`) y las cinco `Session` del camino de notificación por evento en `backend/app/modules/alerts/service.py` (`:116`, `:203`, `:246`, `:268` y `:286`; las de `:335` y `:361` son del barrido de reintentos y quedan fuera) pasan a `run_in_executor`; el despacho del lote sigue siendo secuencial, así que los ítems 40 (FIFO) y 41 (cero duplicados) no se tocan y la ganancia buscada es de solapamiento, no de paralelismo. `backend/app/core/database.py:20-24` crea el engine sin `pool_size` ni `max_overflow`, de modo que rigen los defaults de SQLAlchemy (verificado en ejecución: `QueuePool size=5 overflow_max=10`, 15 conexiones) contra un executor de `min(32, cpu_count + 4)` = 16 hilos en el anfitrión de medición: ambos pasan a configurarse explícitamente y en conjunto, con el executor acotado por el pool. Medición que lo motiva (ítem 43, límite de ingesta 100000/60 s declarado por D38/RN-132): 2.893 eventos en 59,389 s = 48,7 ev/s ≈ 20,4 ms por evento; perfilado en el contenedor con engine y cliente reales — sesión + `SELECT` de agente 1,493 ms, `_get_agent_auth` 1,329 ms, `INSERT` + `commit` 1,564 ms, frente a `XADD` 0,149 ms y `PING` 0,267 ms, más ~5,7 ms de la cadena de notificación que corre en todos los eventos. No migra a `AsyncSession` (RN-76 sigue vigente) y no redefine el umbral de 30 s del ítem 43. Agregada el 2026-09-18. |

| D76 | Completa D75, cuyo executor único —`backend/app/main.py:114-118` construye un `ThreadPoolExecutor` y lo instala con `set_default_executor`, de modo que todo `run_in_executor(None, ...)` del proceso tira del mismo pool— dejó al carril de ingesta y al de notificación compitiendo por el mismo recurso con reglas de admisión opuestas: la ingesta envía secuencialmente, un evento por vez (`events/consumer.py:342`, `:446`, bucle de `:299-300`), y la notificación sin cota desde `_fire_and_forget` (`:497`), encolando entre dos y cinco trabajos por notificación (`alerts/service.py:144`, `:304`, `:327`, `:338`, `:344`, `:357`). Con la cola FIFO del executor sin cota del lado del `submit`, el `_ingest` del evento siguiente espera detrás del backlog de notificaciones. El camino de notificación por evento pasa a un executor dedicado, referenciado explícitamente en sus call sites (`None` queda reservado al carril de ingesta, que conserva el executor por defecto en exclusiva y no cede capacidad); las entregas concurrentes quedan acotadas por un semáforo que cubre las tres puertas de entrada —consumer, `recover_pending_notifications` (cuyo `asyncio.gather` dispara hoy todas las pendientes de golpe) y reintento manual desde la DLQ—, con desborde declarado: la entrega **espera en orden de llegada**, no se descarta ni se difiere, porque descartar violaría al-menos-una-vez y diferir dependería de un barrido periódico que no existe. La creación de la fila `Alert` queda fuera de la cota. El presupuesto de conexiones se valida sobre la **suma** de los dos executors (`10 + 8 ≤ 10 + 18 − 10 = 18`; `db_max_overflow` sube de 10 a 18, el resto sin cambio; 28 conexiones contra el `max_connections=100` por defecto de `postgres:18.3`), en una sola desigualdad y no dos independientes, que admitirían una configuración donde cada executor cabe por su cuenta y juntos agotan el pool. Medición que lo motiva (candidato `v2.0-tesis`, `9c523f4`, paquete `tesis/cierre/evidencia/v2-eval-20260922T175053Z/`, canal SMTP real): notificación aislada 302,8 ms media / 349,4 ms máx (n=10) contra 5.233,672 ms media, p95 6.469,887 y máx 6.532,691 bajo carga de ingesta (n=300) — **~17× de inflación que el mismo canal SMTP no explica**; drenaje con canal real, mediana de tres repeticiones 141,061 s (141,061 / 120,366 / 150,150 s; preservación 100 %, 0 descartados, 0 duplicados) ≈ 18,9 ev/s contra ≈ 49,7 ev/s con canal `log_only` — degradación ≈ 2,6×. Preserva D40/RN-134 y D41/RN-135: `_build_payload` sigue invocándose exactamente una vez por entrega, fuera del bucle, y el permiso de la cota se adquiere una sola vez antes de preparar el payload. No separa la notificación a un stream de Valkey ni a un proceso propio (nombrado como dirección, fuera de alcance), no agrega barrido periódico, no migra a `AsyncSession` y no escala horizontalmente (RN-76). Agregada el 2026-09-22. |

| D77 | `alerts.channel_accepted_at` (`TIMESTAMPTZ` anulable, migración `022_add_alert_channel_accepted_at.sql`, sin backfill ni índice) registra el instante en que un canal aceptó la notificación, distinto de `delivered_at`. Se captura en `notify_event` (`backend/app/modules/alerts/service.py`) con `datetime.now(timezone.utc)` **inmediatamente después** de que `send_n8n(...)` o `_try_fallbacks(...)` devuelven éxito y **antes** del despacho a `run_in_executor`; se pasa como argumento nuevo a `_mark_delivered`, que lo persiste en el **mismo `commit`** que ya escribe `delivered_at` — sin transacción ni despacho adicional al executor. `delivered_at` no cambia de semántica: sigue siendo el `datetime.now(timezone.utc)` evaluado dentro del hilo del executor, al persistir el éxito. Si toda la cascada de canales falla, `channel_accepted_at` permanece `NULL`. No viaja en el payload canónico (D40/RN-134) ni en el modelo de respuesta de `alerts/router.py`; el estado derivado (`delivered`/`failed`/`pending`) sigue derivándose sólo de `delivered_at` y `failed_at`. Motivo medido: sobre el candidato `v3.0-tesis` (`22f393d`, paquete `tesis/cierre/evidencia/v2-eval-20260923T010103Z/`), las medianas de `delivered_at − received_at` de la Batería 4 —13.170 / 11.671 / 10.537 ms— son estrictamente mayores que el intervalo que el protocolo de medición define (`tesis/plan_medicion_cap5.md:202-204`, "recepción del evento → emisión exitosa del webhook"), porque `delivered_at` se escribe después del despacho al executor y del `commit`; la línea `notifier.n8n_sent` (`notifier.py:48`) marca el instante correcto pero no lleva `alert_id` ni `notification_id`, y bajo las hasta 32 entregas concurrentes de D76/RN-170 ningún join por proximidad temporal es una función. Alternativa descartada: que `send_n8n` devuelva el instante exacto del `raise_for_status()` — estructuralmente más preciso, mueve el error residual de microsegundos/milisegundos a cero, pero exige refactorizar la firma de los cuatro canales de la cascada; descartada por desproporción, error residual declarado en el protocolo. Agregada el 2026-09-23. |
| D78 | El arnés de pruebas del backend (`backend/tests/conftest.py`) deja de depender de dos recursos ambientales compartidos, sin tocar código de producción. Caso A — puerto: una fixture `autouse` neutraliza `start_mtls_server` y `start_bootstrap_server` en `app.main` para todo test que corre el lifespan real (`with TestClient(app)`, 41 call sites en `test_notifications.py`, `test_sse_alerts.py`, `test_sse_stream_ticket.py` y `test_logging_sanitize.py`), en vez de editarlos uno por uno; `_free_port()` (promovido desde `test_mtls_transport.py`) se ofrece a los tests que sí necesitan un listener real, con `port=` explícito. `pki.py` conserva `port: int = 8443` / `port: int = 8444` como default y `main.py` conserva su invocación del lifespan sin `port` — verificado con tests de guarda dedicados. Caso B — dotenv: `backend/tests/conftest.py` monkeypatchea `DotEnvSettingsSource._read_env_files` (pydantic-settings) para que devuelva siempre `{}`, antes del primer import de `app.core.config`; `env_file=".env"` (`config.py:40`) sigue intacto, pero la suite deja de heredar un `.env` real del directorio desde el que se la invoca — `monkeypatch.delenv` sobre `os.environ`, que los tests de configuración ya hacían, sanea el proceso pero no el archivo. Motivo medido, sobre el artefacto sellado (`tesis/cierre/evidencia/v2-eval-20260923T010103Z/suites/`, `candidate_tag=v3.0-tesis`, `candidate_commit=22f393d`): backend 864 tests, 15 fallas, 4 omitidos — 13 `RuntimeError: This portal is not running` (`test_notifications` 7, `test_sse_alerts` 6, por `OSError: [Errno 98] address already in use` sobre 8443 con cascada al portal anyio compartido) y 2 `AssertionError` de configuración heredada (`test_notification_settings`); agente 642/0/1 y frontend 260/0/0, sin cambios. La misma suite contra contenedores efímeros con el puerto libre y sin `.env` en el CWD reporta 0 fallas — eso no prueba la corrección, prueba que ese entorno evita el conflicto. Agregada el 2026-09-23. |
