...
- **M4 (MEDIO)** — ✅ **D68/RN-162 cerrada el 2026-07-02** — `agents/heartbeat_consumer.py` `_sweep_offline`: `Agent.last_heartbeat < threshold` excluye filas `NULL` (en SQL `NULL < x` es NULL) → agentes que nunca latieron nunca pasan a `offline`/`dead`. Estuvo bloqueado en el apply de C34 (2026-07-02) porque el modelo `Agent` no tenía `created_at` ni `registered_at`, sin referencia temporal para decidir cuándo un agente que nunca latió pasa a `dead`. **D68/RN-162** cierra el gap (decisión: opción B con `registered_at` grace period; ver appendix "Decisiones de implementación — Abril 2026" en `docs/reglas_de_negocio.md`): se agrega `Agent.registered_at` con migración `019_add_agent_registered_at.sql`, y el barrido marca `dead` cuando `now - registered_at` supera el mismo umbral de `offline → dead` (300 s / 5 min). M4 queda desbloqueado.
...
> **Follow-ups**: los tres identificados en la proposal (hash post-restauración tautológico, dos implementaciones divergentes de cuarentena, `cp -r` no idempotente de `install.sh`) **no** se abrieron como changes: quedaron registrados en [`docs/residuales_declarados.md`](docs/residuales_declarados.md) junto con los hallazgos de la verificación contra el VPS y el host del agente del 2026-09-16.
...
> **Absorbido por el change 52 (`vps-deployment-readiness`) el 2026-09-12.** No se abre como change propio: su alcance completo se implementa y verifica dentro del 52. La descripción de abajo sigue siendo la referencia de alcance.
...
> **Deuda transitoria — cerrada el 2026-09-13 al aplicar el change 52.** Al subir el pin en los docs prospectivos (D45/RN-139) había quedado una inconsistencia temporal — `tesis/entrega_valores_cap5.md:214` verificaba «declarado vs real» citando `docs/arquitectura_stack.md:44` (ya en 2.17.8) contra `docker-compose.yml:85` (todavía 2.16.1). El change 52 subió el pin real de `docker-compose.yml` a `n8nio/n8n:2.17.8` (servicios `n8n` y `n8n-provision`) y actualizó la cita de esa fila; ambos quedan en 2.17.8 y la fila es coherente. Los **valores medidos** del Cap. 5 no se tocaron — la fila lo declara explícitamente.
...
**Capa**: backend · **Depende de**: 06 (`agent-mtls-bootstrap`) · **Origen**: barrido spec↔código del 2026-08-24, sobre los 98 requisitos que estuvieron invisibles por el daño de specs · **Decisiones**: D67/RN-161 (auditoría de acciones originadas por el sistema)

**Done** ✅ (2026-09-16): **la premisa de la nota siguiente quedó superada.** El endpoint se implementó en `28f87fe`, casi en paralelo a esta propuesta, y el `tasks.md` nunca lo reflejó: al aplicar el change, el barrido contra el código encontró ya resueltos el endpoint sobre el listener mTLS, la emisión sobre clave pública, los dos chequeos de revocación, el contrato agente↔backend y el bloque de vencimiento fail-closed. El hueco real era la **auditoría**, imposible de escribir porque `audit_log.user_id` era un FK no nulo a `users` y una renovación no tiene usuario detrás: se cerró con D67/RN-161 (`user_id` nullable, actor y seriales en `extra`) y la migración `018`. Se agregaron además el test de que la renovación no revoca el certificado saliente y un end-to-end que renueva a 10 días del vencimiento y reabre una segunda conexión mTLS con el certificado nuevo y la clave sin rotar. La clave privada **no** rota (7.2): el mTLS ya prueba posesión, y rotarla exigiría CSR y reabriría el problema de confianza del bootstrap. Suite backend: 816 passed / 4 skipped.
...
> **Camino sin salida — RESUELTO por D48/RN-142.** Un agente cuyo certificado ya venció no tenía vuelta por ningún lado: renovar falla en el handshake, re-bootstrapear da `401` (el secret se destruye al consumirse, `agents/service.py:80`) y re-registrar da `409`. Y el fondo era peor que la conectividad: el `master_secret` **no se persiste en el backend** —vive sólo en la respuesta del bootstrap, nunca en la tabla `Agent`, por D1—, así que un re-bootstrap emite uno nuevo y **destruye el baseline cifrado del agente**. Se resuelve con una **ventana de gracia de 30 días** sobre `not_valid_after`: un certificado vencido sigue siendo prueba de posesión de la clave privada, y lo que acota el riesgo es la verificación de revocación, no la fecha.

> **La relajación está confinada, y eso es criterio de aceptación.** El puerto 8443 es compartido, así que en vez de agregar una excepción se **invierte el default**: el listener pasa a `CERT_OPTIONAL` y una dependencia de validación que **falla cerrada** —certificado presente, cadena válida, CN coherente, no vencido, no revocado— se vuelve obligatoria para todos los endpoints mTLS. `/agents/renew` es la única excepción y sólo sobre el vencimiento. Un endpoint nuevo que olvide declararla queda **protegido**, no expuesto. Incluye test de no-regresión: los demás endpoints siguen rechazando un certificado vencido.
...
### Change 51 — `agent-attribution-and-detection-gap`

**Capa**: agente + backend + frontend + base · **Depende de**: 41 (`agent-deployment-caps`, que fijó el contrato de capabilities y despliegue del agente) y 45 (`frontend-severity-triage`, que dejó la severidad visible y filtrable en la consola de triage) · **Origen**: auditoría externa de la tesis del 2026-08-21, hallazgos verificados contra el código el 2026-08-24 · **Decisiones**: D49 (RN-143), D50 (RN-144), D51 (RN-145)

> **Nota**: **el sistema atribuye a root los cambios cuyo autor no supo identificar, y no ve cuando el kernel le esconde eventos.** Dos defectos independientes que comparten una misma forma: el agente convierte «no sé» en una afirmación positiva. `agent/detector.py:98-106` — `_get_uid` está declarada `-> int` y devuelve `0` en `OSError`; `/proc/<pid>/status` desaparece en cuanto el proceso termina, y entre la notificación del kernel y el armado del evento hay hasta 150 ms de reintentos de hash, así que un editor que escribe y sale, un `install` o un paso de gestor de paquetes **ya no existe** cuando se lo consulta. `agent/decision.py:151` es peor: la ruta de rehidratación del journal escribe `"process_uid": 0` **incondicionalmente**, de modo que todo evento recuperado tras un reinicio del agente se reporta como hecho por root. Y `agent/_fanotify.py` **nunca define la constante `FAN_Q_OVERFLOW`** (`0x00004000`): `_parse_events` (`:241-258`) no la chequea, así que cuando el kernel descarta eventos por saturación de su propia cola el agente no se entera y no puede inferir que faltaron.

**Por qué no se detectó antes**: los dos defectos son invisibles a la suite. El `0` de `_get_uid` es un entero válido que el backend acepta sin chistar —la columna es `int | None` desde siempre— y ningún test asserta sobre `process_uid` en la ruta de rehidratación (`agent/tests/test_decision.py:220,364,399,419` ejercen `engine.rehydrate()` y ninguno lo mira). Las fixtures con `process_uid: 0` que sí existen (`test_decision.py:60-77`, `test_restore_metadata.py:77`, `test_symlink_hardening.py:565`) representan un proceso root **real** y son correctas — no son evidencia de cobertura del defecto. El desbordamiento del kernel, por su parte, es silencioso **por construcción**: no hay señal que observar, que es justamente el hallazgo. El tercer punto es documental y sobrevivió por la misma razón: `docs/arquitectura_stack.md:1509` afirmaba que el agente «monitorea activamente el nivel de backpressure y lo reporta como anomalía en el heartbeat (`queue_pressure` flag)», y `queue_pressure` es la cola offline de 100 MB en disco (`agent/queue.py`), sin ninguna relación con la saturación de `fanotify`. La afirmación era falsa y hacía parecer cubierto lo que no lo estaba.

Capacidades:
- **Atribución honesta en el agente (D49/RN-143)**: `_get_uid` pasa a `-> int | None` y devuelve `None` cuando no puede leer `/proc/<pid>/status`; `FanotifyEvent.uid` y `DetectedChange.process_uid` acompañan el cambio de tipo. La rehidratación del journal (`agent/decision.py:151`) emite `process_pid`, `process_uid` y `process_exe` en `null` en vez de `0` — ahí el proceso original no existe **por definición**. Sin migración: el backend y el frontend ya son null-safe (`events/models.py:58`, `EventOut.process_uid`, `frontend/src/api/events.ts`).
- **`FAN_Q_OVERFLOW` deja de ser invisible (D50/RN-144)**: se define la constante y `_parse_events` la reconoce; el hilo lector emite un evento sintético `event_type=detection_gap` con `path` nulo, causa `fan_q_overflow` y contexto de proceso nulo, por el **mismo stream** que la evidencia. Deduplicación **por ventana**: a lo sumo un `detection_gap` por ventana de 60 s, con la cuenta de desbordamientos suprimidos en el propio evento — inundar la tabla justo cuando el sistema está saturado convertiría al evento diagnóstico en una segunda falla.
- **`event_type` persistido y `path` opcional (D51/RN-145)**: columna `event_type` en `Event` con el vocabulario que el agente ya emite (`file_created`, `file_modified`, `file_deleted`, `file_absent`, `detection_gap`), en snake_case por RN-71, propagada a `EventOut`, a `EventListItem` y al tipo del frontend. `Event.path` pasa a nullable, y la ingesta deja de degradar el nulo a `""` (`events/service.py:164`, `event_data.get("path", "")`).
- **Un evento sin ruta no participa de la supersesión por path**: `get_pending_event_for_path` y `compact_chain` se saltean cuando la ruta es nula. Con la cadena vacía de hoy, **todos** los `detection_gap` se supersederían entre sí como si fueran el mismo archivo, y cada brecha nueva borraría la anterior de la vista del operador — precisamente el dato que D50 existe para preservar.
- **Severidad de un evento sin ruta**: excepción acotada a D34/RN-128 — el backend sigue siendo la única autoridad y la sigue calculando al ingerir, pero un evento sin ruta recibe `high` fijo sin consultar el ruleset. La lógica vigente deriva la severidad de las reglas que matchean el path, y un evento sin path no matchea ninguna: caería en `low`, que es lo contrario de lo que significa. **Hallazgo del apply (no anticipado por el design)**: `alerts/service.py::_determine_severity` recalculaba la severidad de forma independiente para decidir si notificar — mismo `path=None` sin guardar, mismo riesgo. Se aplicó la misma excepción ahí.
- **Motor de decisión y UI**: un evento sin ruta no puede restaurarse ni ponerse en cuarentena, así que entra como `alert_only` sin tocar el resto del motor; `EventsTable` y `EventDetail` renderizan la fila sin ruta sin guiones ni vacíos, con el tipo de evento como discriminador visible.
- **Migración de base en esta change** (`012`, misma convención SQL manual idempotente que 005–011): `event_type` NOT NULL con default `file_modified` para las filas previas, y `path` relajada a nullable.
- **Limpieza de `pyfanotify`**: referencias stale en `agent/detector.py:2,52,242,279,363` y en los docstrings de `test_symlink_hardening.py:22`, `test_detector_multi_event.py:39`, `test_scope_filter.py:14,195`, `test_agent_config.py:17`. **No** se tocan `agent/requirements.txt:6` ni `agent/_fanotify.py:5`: el primero ya dice explícitamente que no se depende de `pyfanotify` y el segundo es una referencia legítima al wrapper descartado.

Reglas: RN-143 (atribución no resuelta), RN-144 (brecha de detección como evento), RN-145 (`event_type` persistido, `path` opcional, severidad sin ruta), RN-71 (léxico snake_case), RN-128 y D34 (el backend es la autoridad sobre la severidad), RN-98 (compactación de cadena).

> **Diseño**: no existe un valor de relleno aceptable para `process_uid`. Cualquier entero que se elija colisiona con un uid real, y el que estaba elegido —`0`— colisiona con el **peor** posible: un operador que filtra por `uid = 0` para buscar cambios privilegiados recibía una lista contaminada con todo cambio cuyo autor el sistema no supo identificar, sin ninguna marca que los distinguiera. `null` no es un default conservador que se pueda mejorar más adelante; es la única representación que no miente.

> **Por qué un evento y no una métrica.** Una métrica se mira cuando alguien ya sospecha; un evento aparece en la bandeja de trabajo y fuerza el triage. La asimetría es lo que importa: un `detection_gap` no dice «cambió un archivo», dice «entre estos dos instantes pude haber perdido cambios». Es una afirmación sobre la confianza en el **resto** de la evidencia, y por eso tiene que viajar por el mismo canal que la evidencia. `high` y no `critical`: una brecha de detección es una pérdida de garantía, no una violación de integridad confirmada, y reservar `critical` para lo confirmado mantiene informativa a la severidad máxima.

> **Impacto en los resultados de la tesis.** Los datos de `tesis/resultados/` corresponden al esquema **previo** a la migración `012` y a un agente que atribuía a root la atribución no resuelta. La **re-corrida de las baterías del Capítulo 5 es condición para reportar** — no es una mejora opcional de los números, es que las afirmaciones sobre fiabilidad de la atribución de procesos no son sostenibles contra el esquema viejo. Aceptado explícitamente al aprobar esta change.

**Done**: `_get_uid` sobre un pid inexistente devuelve `None` y hay test que lo fija; un test sobre `engine.rehydrate()` asserta `process_uid is None` (hoy esa línea no tiene cobertura alguna); un `FAN_Q_OVERFLOW` sintético produce exactamente un evento `detection_gap` por ventana de 60 s con la cuenta de supresiones; dos `detection_gap` consecutivos coexisten en la base sin supersederse; un evento sin ruta ingresa con severidad `high` y estado `alert_only`; `event_type` viaja del agente a la UI de punta a punta; y la migración `012` corre dos veces sin error.

---

### Change 52 — `vps-deployment-readiness`

**Capa**: infra + backend + agente + frontend + n8n + docs · **Depende de**: 41 (`agent-deployment-caps`, que fijó el layout de `install.sh` y el drop-in de `watch_paths`) y D52/RN-146 (listener de bootstrap 8444, ya implementado) · **Absorbe**: 47 (`n8n-operable-workflows`), completo · **Coordina con**: 50 (`backend-agent-cert-renewal`, en curso, también toca `backend/app/core/pki.py`) · **Origen**: ensayo multi-host A-3 (2026-09-12) y relevamiento de reproducibilidad del mismo día · **Decisiones**: D53/RN-147, D54/RN-148, D55/RN-149, D56/RN-150; aplica D41/RN-135, D43/RN-137, D44/RN-138, D45/RN-139

> **Nota**: **el sistema funciona entre dos equipos, pero sólo con pasos que no están en el producto.** El ensayo A-3 lo demostró y dejó registrado el costo: los SAN de los certificados están fijos en código (`pki.py:161` → `backend, fim-backend, localhost`; `emitir_cert_valkey.py:72` → `valkey, localhost`), así que un agente que apunta a la IP o al dominio del servidor falla la verificación de hostname y hubo que mapear nombres en `/etc/hosts` del host monitoreado. El 6380 sólo se publica en un override de evidencia; el certificado de cliente del backend ante Valkey se emitió con un script de la carpeta de evidencia; el compose publica 8000 en `0.0.0.0`; `install.sh` no recibe ningún valor y re-ejecutarlo anida el código nuevo en `/opt/fim-agent/agent/agent/`; el registro del agente usó un script ad hoc. Y n8n arranca con **0 workflows** y `N8N_HEALTH_URL` vacía, por lo que `GET /health/components` lo reporta `degraded` —correctamente, por D43— y ninguna alerta sale por ese canal.

Capacidades:
- **SAN configurable (D53/RN-147)**: `FIM_PUBLIC_HOSTS` en `.env` extiende el SAN de los certificados de backend y Valkey, con reemisión idempotente; emisión automática del certificado de Valkey y del de cliente del backend (`CN=fim-backend-valkey`) antes de que Valkey arranque con TLS.
- **Topología de servidor sin editar YAML (D54/RN-148)**: `docker-compose.tls.yml` publica 6380; 8000 deja de publicarse en `0.0.0.0`; script de preparación que genera `.env` sin sobrescribir uno existente.
- **Consola con HTTPS opcional (D55/RN-149)**: modos sin TLS, autofirmado o certificado provisto; `Secure` de la cookie de refresh y HSTS atados al modo, no a `ENVIRONMENT`.
- **Instalador del agente parametrizable (D56/RN-150)**: flags o variables de entorno con prompts para lo faltante, secreto sólo por prompt oculto o archivo, huella SHA-256 de la CA verificada, reinstalación que reemplaza el código, verificación de alcance a 8444 y 6380; paso único del lado del servidor que registra el agente y entrega host, huella y secreto.
- **n8n operable (absorbe el change 47 completo)**: pin a 2.17.8 con owner declarativo (D45/RN-139), `N8N_ENCRYPTION_KEY`, healthcheck, provisioning idempotente del enrutador `fim-alert` y sus sub-flujos (D44/RN-138) con activación verificada, expresiones corregidas, search-before-create en ticketing (D41/RN-135). Ver el alcance detallado en el change 47.
- **Specs desactualizadas**: `infra-compose` (pin de n8n, puerto 8444, override TLS, exposición de puertos, provisioning de n8n), `backend-pki` (SAN del certificado del backend y de Valkey), `agent-valkey-transport` (verificación de hostname contra el SAN), `agent-bootstrap` (listener 8444 y ancla de confianza inicial), `agent-core` (contrato del instalador), `backend-agents` (bootstrap fuera del puerto 8000, D52 — desvío registrado en A-3). El delta se escribe por el CLI, nunca a mano (D47/RN-141).
- **Guía de despliegue de producto**: servidor remoto + agentes remotos, fuera de `tesis/cierre/evidencia/`; `README.md` enlaza a ella.

Reglas: RN-76, RN-78, RN-114, RN-115, RN-52, RN-53, RN-146, RN-147 a RN-150, D-04.

> **Coordinación con el change 50**: los dos tocan `backend/app/core/pki.py`, en funciones distintas (`_ensure_backend_cert` acá; emisión sobre clave pública allá) y ambos pueden aportar delta a `backend-pki`. Quien archive segundo re-verifica el delta contra la main spec ya actualizada, con `scripts/check_spec_integrity.py` antes y después.

**Done**: en un servidor con `FIM_PUBLIC_HOSTS=<ip>` y el stack levantado con los dos archivos de compose sin editar YAML, un agente instalado en otro host con `install.sh` y los datos entregados por el servidor —sin tocar `/etc/hosts` ni editar archivos— completa el bootstrap por 8444, publica por 6380 y sus eventos aparecen en la consola; la consola funciona por HTTP sin dominio y por HTTPS con certificado autofirmado, con el refresh de sesión operativo en ambos modos; desde fuera del servidor 8000, 5432, 6379 y 5678 no son alcanzables; re-ejecutar `install.sh` actualiza el código instalado; `GET /health/components` reporta n8n `ok` y un `POST` real a `/webhook/fim-alert` ejecuta el enrutador; `openspec validate --specs` y `scripts/check_spec_integrity.py` pasan.
...
- **Agente (D73/RN-167, hallazgo post-despliegue)**: `agent/logging.py` nunca filtró la salida de `structlog` por nivel, así que el `debug` de arriba no tuvo efecto real y el journal siguió inundado (82 líneas `debug` medidas en 15 s tras el despliegue del 2026-09-17). `configure_logging` pasa a `structlog.make_filtering_bound_logger`, mismo mecanismo que el backend.
- **Agente (D74/RN-168, segundo hallazgo post-despliegue)**: con el filtro de nivel ya funcionando, el journal seguía recibiendo `detector.event_null_path` a `warning` (71.833 líneas entre las 13:00 y las 14:44 del 2026-09-17, ~12/s; 135.547 desde el 2026-09-16). Causa raíz investigada en `agent/_fanotify.py`: `_open_by_handle` devuelve `None` (típicamente `ESTALE`) cuando el handle del kernel ya no resuelve, en un punto anterior al filtro de scope. A diferencia de `out_of_scope_drop`, no hay certeza de que el objeto perdido estuviera fuera de `watch_paths` — el contador nuevo `null_path_drops` se documenta como posible brecha de cobertura, no como ruido confirmado. El log baja a `debug` y el contador viaja en el heartbeat, sin persistencia en backend ni presentación en frontend (fuera de alcance de esta decisión).
...
**Orden de despliegue (restricción, no preferencia)**: el slice del agente va **antes** de re-correr las baterías del Capítulo 5 (tarea 12.4 del change 51); los de backend y frontend no tocan `agent/`, así que pueden ir después sin invalidar la corrida. El redespliegue con D73/RN-167 se hizo el 2026-09-17 17:57:16 UTC y se verificó en el journal (tarea 1.12); el redespliegue con D74/RN-168 queda pendiente (tarea 1.16).

Reglas: RN-04 (ratificada sin cambio), RN-71, RN-92. Decisiones aplicadas: **D69/RN-163**, **D73/RN-167**, D3 (migración aplicada a mano), D46/RN-140 (modo FID, causa raíz del volumen), D37/RN-131 (contraste de presentación, sin alterar).

**Done**: el journal del agente deja de recibir un warning por ruta descartada y la sección 4.2 de `agent/tests/test_scope_filter.py` sigue verde sin tocar el archivo; a nivel `info` ningún log `debug` —incluido `detector.out_of_scope_drop`— llega al journal, verificado tras el redespliegue de D73/RN-167; un heartbeat con el contador lo persiste y los dos endpoints lo exponen; una clave ausente no resetea el valor guardado y un agente que nunca reportó lee `null`, no `0`; la tarjeta del agente muestra el contador con tratamiento neutro, distinguible del indicador de descartes locales, y distingue "nunca reportó" de cero.
...
- **Trazabilidad**: `tesis/cierre/MATRIZ_TRAZABILIDAD.md` y `tesis/trazabilidad_us_tests.md` a 31/0/0 con la tabla de ajustes declarados (texto original, texto vigente, decisión, fecha, commit).
...
### Change 58 — `ingest-offload-blocking-db`

**Capa**: backend · **Depende de**: 30 (`backend-async-io-fixes`, dueña de D21, archivada) y 51 (`agent-attribution-and-detection-gap`, de donde sale el instrumental de medición) · **Paralelizable con**: 53, 54 · **Origen**: medición del ítem 43 del 2026-09-18 sobre el candidato `v1.0-tesis` (`devel`, `7a906c2`), drenaje 2.893 eventos en 59,389 s = 48,7 ev/s contra un umbral de < 30 s · **Decisiones**: D75/RN-169 (amplía D21); no modifica D38/RN-132

> **Nota**: el drenaje tras reconexión no llega al umbral del ítem 43 y el broker no es el cuello. El perfilado dentro del contenedor, con el engine y el cliente reales, atribuye los ~20,4 ms por evento a operaciones de base de datos **bloqueantes serializadas dentro del event loop**: sesión + `SELECT` de agente 1,493 ms, `_get_agent_auth` 1,329 ms, `INSERT` + `commit` 1,564 ms, frente a `XADD` 0,149 ms y `PING` 0,267 ms. El camino de **rechazo** ya saca su trabajo del loop (`consumer.py:420`, `:563`); el camino **feliz** no. Esta change corrige esa asimetría según D75/RN-169.

> **El despacho sigue siendo secuencial.** No hay `gather` ni concurrencia entre eventos: los ítems 40 (FIFO) y 41 (cero duplicados) quedan intactos. La ganancia buscada es de **solapamiento** —que la cadena de notificación de eventos previos avance mientras la ingesta del actual espera a la base en un hilo—, no de paralelismo. Mover el trabajo al executor sin tocar el pool cambiaría un cuello por agotamiento de conexiones, que además falla en vez de degradar: hoy son 15 conexiones (`QueuePool size=5 overflow_max=10`, verificado en ejecución) contra 16 hilos de executor en el anfitrión de medición.

Capacidades:
- **Carril feliz no bloqueante**: `_get_agent_auth` (`consumer.py:305`), `_ingest` (`:400`) y la `Session` propia de `notify_if_applicable` (`alerts/service.py:116-119`) pasan a `run_in_executor`, replicando el patrón que el carril de rechazo ya usa. Las funciones síncronas no cambian de interfaz (mismo criterio que D21).
- **Pool y executor dimensionados en conjunto**: `pool_size` y `max_overflow` explícitos en `backend/app/core/database.py`, y un executor dedicado con el número de hilos acotado por `pool_size + max_overflow` descontando lo que consumen las dependencias HTTP de FastAPI y el consumer de heartbeat. Ambos valores configurables.
- **Sin migración a `AsyncSession`**: el fundamento de D21 sigue vigente por RN-76 (backend single-instance).
- **Medición posterior prevista**: re-correr la Batería 5 (corte de Valkey, límite efectivo declarado, drenaje medido desde `received_at` en la base y **no** desde el sondeo del arnés) y registrar el resultado.

Reglas: RN-169 (nueva), D21 (ampliada), RN-76 (single-instance, sin escalado horizontal), D38/RN-132 (el default de producción del rate limit **no** se cambia), ítems 40 y 41 del protocolo (no degradables). Fuera de alcance: redefinir el umbral de 30 s del ítem 43 — si tras medir el drenaje sigue por encima, eso abre un ajuste de criterio declarado propio con número nuevo.

**Done**: el carril feliz del consumer no abre ninguna `Session` síncrona dentro del event loop; el despacho del lote sigue siendo mensaje a mensaje y los tests de FIFO y de no-duplicación pasan sin cambios; `pool_size`, `max_overflow` y el tamaño del executor son explícitos y configurables, con el executor acotado por el pool; la suite de backend pasa; `scripts/check_spec_integrity.py` pasa; queda registrada la tarea de re-medición de la Batería 5 con su resultado, sin declarar incumplimiento anticipado del ítem 43.
...
### Change 60 — `notify-accepted-at-and-test-port-isolation`

**Capa**: backend + arnés de pruebas · **Depende de**: 59 (`notify-isolate-executor-lane`, dueña de D76/RN-170, del cupo de entregas concurrentes y de la forma actual de `notify_event`, archivada) y 58 (`ingest-offload-blocking-db`, dueña de D75/RN-169 y del despacho de `_mark_delivered` al executor, archivada) · **Paralelizable con**: ninguna que toque `backend/app/modules/alerts/service.py`, `backend/app/modules/alerts/models.py` o `backend/tests/conftest.py` · **Origen**: auditoría del capítulo de evaluación sobre el candidato `v3.0-tesis` (`22f393d`), paquete `tesis/cierre/evidencia/v2-eval-20260923T010103Z/` · **Decisiones**: D77/RN-171 (marca durable de aceptación del canal, distinta de `delivered_at`); D78/RN-172 (aislamiento de la suite de recursos ambientales, sin tocar producción); preserva D76/RN-170, D75/RN-169, D40/RN-134, D41/RN-135 y D39/RN-133

> **Nota**: dos defectos de **instrumentación**, no de producto en funcionamiento, que viajan juntos porque el capítulo de evaluación los declara en la misma página. Defecto 1: el intervalo que el protocolo de medición define —*"recepción del evento → emisión exitosa del webhook"*, camino feliz, sin reintentos— no se registra en ninguna parte; lo único que existe es `delivered_at`, que se escribe **después** del retorno del envío, del despacho al executor y del `commit`, y produce un intervalo estrictamente mayor. Defecto 2: la suite de backend colisiona consigo misma sobre dos recursos ambientales compartidos —el puerto 8443 cableado en `pki.py` y el archivo `.env` que `config.py` resuelve contra el directorio de trabajo—, y esas fallas no dicen nada del producto.

> **Evidencia.** Defecto 1: medianas de `delivered_at − received_at` de **13.170 / 11.671 / 10.537 ms** en los tres escenarios de la Batería 4; la línea `notifier.n8n_sent` marca el instante correcto pero no lleva `alert_id`, y bajo las hasta 32 entregas concurrentes de D76/RN-170 ningún join por proximidad temporal es una función. Defecto 2, sobre el artefacto sellado: backend 864 tests, **15 fallas**, 4 omitidos — **13** `RuntimeError: This portal is not running` (`test_notifications` 7, `test_sse_alerts` 6) y **2** `AssertionError` de configuración heredada (`test_notification_settings`); agente 642/0/1 y frontend 260/0/0, sin cambios. La misma suite contra contenedores efímeros reporta 0 fallas — eso no prueba la corrección, prueba que ese entorno evita el conflicto.

> **Principio que gobierna la change**: una medición es creíble cuando el número que se informa es el número que se definió, y cuando una falla reportada dice algo del producto.

Capacidades:
- **Marca durable de aceptación del canal**: `alerts.channel_accepted_at` (`TIMESTAMPTZ` anulable, migración `022`, sin backfill ni índice), capturada en la corrutina de `notify_event` inmediatamente después de que el envío devolvió éxito y antes de despachar nada al executor, persistida en el **mismo `commit`** que ya escribe `delivered_at`. `delivered_at` **no cambia de semántica** — se agrega una marca, no se reinterpreta una.
- **La marca no crece la superficie de producto**: no viaja en el payload canónico de notificación (D40/RN-134) ni en el modelo de respuesta de `alerts/router.py`; el estado derivado (`delivered`/`failed`/`pending`) sigue derivándose sólo de `delivered_at` y `failed_at`.
- **Puerto del servidor mTLS aislado en el arnés, sin tocar producción**: una fixture `autouse` centralizada en `backend/tests/conftest.py` neutraliza `start_mtls_server` y `start_bootstrap_server` para los 41 call sites `with TestClient(app)`, en vez de editarlos uno por uno; `_free_port()` se promueve al conftest para los tests que sí necesitan un listener real. `pki.py` y `main.py` quedan **byte-idénticos**.
- **Entorno de configuración determinado en el arnés**: `backend/tests/conftest.py` neutraliza la lectura del `.env` que `config.py` resuelve contra el CWD, una sola vez, antes del primer import de la aplicación. `config.py` queda **byte-idéntico**.
- **Invariantes de D76/RN-170 preservados y verificados**: `notification_id` estable a lo largo de toda la escalera (`_build_payload` una sola vez, único cambio de firma en `_mark_delivered`), al-menos-una-vez, cascada de canales, `RETRY_DELAYS`, el permiso del cupo de entregas adquirido una sola vez antes de `_prepare_notification`.
- **Re-medición obligatoria**: esta change **invalida el candidato congelado `v3.0-tesis`**. Se re-corre el arnés unificado completo (`~/fim-lab/corrida_unificada.sh`) y se emite un **tag nuevo**; el paquete `v2-eval-20260923T010103Z` queda como línea de base de comparación.

Reglas: RN-171 (nueva), RN-172 (nueva), D76/RN-170, D75/RN-169, D40/RN-134, D41/RN-135 y D39/RN-133 (preservadas, no reabiertas), D43/RN-137 (la defensa que `test_notification_settings.py` sostiene se conserva intacta). Fuera de alcance, **declarado como dirección y no como omisión**: ligar `notifier.n8n_sent` a la alerta (la columna durable vuelve innecesario el join) y convertir el puerto mTLS en un parámetro de despliegue (toca el contrato del agente, otra magnitud de cambio).
