...
> **Impacto en los resultados de la tesis.** Los datos de `resultados/` corresponden al esquema **previo** a la migración `012` y a un agente que atribuía a root la atribución no resuelta. La **re-corrida de las baterías del Capítulo 5 es condición para reportar** — no es una mejora opcional de los números, es que las afirmaciones sobre fiabilidad de la atribución de procesos no son sostenibles contra el esquema viejo. Aceptado explícitamente al aprobar esta change.
...
- **Guía de despliegue de producto**: servidor remoto + agentes remotos, fuera de `docs/cierre/evidencia/`; `README.md` enlaza a ella.
