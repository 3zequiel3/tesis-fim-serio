...
- **Severidad de un evento sin ruta**: excepción acotada a D34/RN-128 — el backend sigue siendo la única autoridad y la sigue calculando al ingerir, pero un evento sin ruta recibe `high` fijo sin consultar el ruleset. La lógica vigente deriva la severidad de las reglas que matchean el path, y un evento sin path no matchea ninguna: caería en `low`, que es lo contrario de lo que significa.
