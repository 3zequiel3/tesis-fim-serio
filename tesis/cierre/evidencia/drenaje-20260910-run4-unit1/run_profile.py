...
class InstrumentedValkey:
    ...
    def pipeline(self, *args: Any, **kwargs: Any) -> Any:
        return InstrumentedPipeline(self, self._client.pipeline(*args, **kwargs))
    ...
    async def xadd(self, stream: str, fields: dict[str, Any], *args: Any, **kwargs: Any) -> Any:
        ...
        if isinstance(event_id, str):
            ...
            if stream == "events":
                row["event_xadd_count"] = row.get("event_xadd_count", 0) + 1
                row.setdefault("xadd_start_ns", started)
                row.setdefault("xadd_end_ns", finished)
                row.setdefault("stream_id", str(result))
            elif stream == "commands" and payload.get("type") == "event_ack":
                row["event_ack_xadd_count"] = row.get("event_ack_xadd_count", 0) + 1
                row.setdefault("event_ack_xadd_start_ns", started)
                row.setdefault("event_ack_xadd_end_ns", finished)
        return result
        ...
        for _stream, messages in result or []:
            for msg_id, fields in messages:
                ...
                if isinstance(event_id, str):
                    ...
                    row = self._metrics[event_id]
                    row["consumer_delivery_count"] = row.get("consumer_delivery_count", 0) + 1
                    row.setdefault("consumer_read_ns", observed)
        return result
    ...
    async def xack(self, stream: str, group: str, msg_id: str, *args: Any) -> Any:
        ...
        if event_id:
            row = self._metrics[event_id]
            row["xack_count"] = row.get("xack_count", 0) + 1
            row.setdefault("xack_start_ns", started)
            row.setdefault("xack_end_ns", finished)
        return result


class InstrumentedPipeline:
    """Observe the atomic event_ack + XACK transaction added by Unit 1."""

    def __init__(self, owner: InstrumentedValkey, pipeline: Any) -> None:
        self._owner = owner
        self._pipeline = pipeline
        self._ack_event_ids: list[str] = []
        self._xack_event_ids: list[str] = []

    def __getattr__(self, name: str) -> Any:
        return getattr(self._pipeline, name)

    def xadd(self, stream: str, fields: dict[str, Any], *args: Any, **kwargs: Any) -> InstrumentedPipeline:
        raw = fields.get("data")
        if stream == "commands" and isinstance(raw, str):
            try:
                payload = json.loads(raw)
            except json.JSONDecodeError:
                payload = {}
            ...
            if payload.get("type") == "event_ack" and isinstance(event_id, str):
                self._ack_event_ids.append(event_id)
        self._pipeline.xadd(stream, fields, *args, **kwargs)
        return self

    def xack(self, stream: str, group: str, msg_id: str, *args: Any) -> InstrumentedPipeline:
        event_id = self._owner._stream_message_to_event.get(str(msg_id))
        if event_id:
            self._xack_event_ids.append(event_id)
        self._pipeline.xack(stream, group, msg_id, *args)
        return self

    async def execute(self) -> Any:
        started = self._owner._clock()
        result = await self._pipeline.execute()
        finished = self._owner._clock()
        # Redis executes this pipeline as one transaction. Both operations
        # therefore share the observable transaction boundary; their internal
        # ordering is not separately measurable from the client.
        for event_id in self._ack_event_ids:
            row = self._owner._metrics[event_id]
            row["event_ack_xadd_count"] = row.get("event_ack_xadd_count", 0) + 1
            row.setdefault("event_ack_xadd_start_ns", started)
            row.setdefault("event_ack_xadd_end_ns", finished)
        for event_id in self._xack_event_ids:
            row = self._owner._metrics[event_id]
            row["xack_count"] = row.get("xack_count", 0) + 1
            row.setdefault("xack_start_ns", started)
            row.setdefault("xack_end_ns", finished)
...
async def main() -> int:
    ...
    def measured_verify(secret: bytes, payload: dict[str, Any]) -> bool:
        ...
        if result and isinstance(event_id, str):
            metrics[event_id].setdefault("hmac_verified_ns", clock())
        return result
    ...
    def measured_ingest(
        ...
        event_id = payload.get("event_id")
        if isinstance(event_id, str):
            metrics[event_id].setdefault("persist_start_ns", clock())
        result = original_ingest(payload, received_at, detected_at, agent_id)
        if isinstance(event_id, str):
            metrics[event_id].setdefault("persist_commit_ns", clock())
        return result
    ...
    async def measured_handle_command(payload: dict[str, Any]) -> None:
        ...
        event_id = payload.get("event_id")
        if payload.get("type") == "event_ack" and isinstance(event_id, str):
            metrics[event_id].setdefault("agent_ack_applied_ns", clock())
    ...
    summary = {
        "evaluation": "drenaje-20260910-run4-unit1",
        ...
        "event_stream_messages": sum(row.get("event_xadd_count", 0) for row in rows),
        "duplicate_event_stream_messages": sum(max(row.get("event_xadd_count", 0) - 1, 0) for row in rows),
    ...
    environment = {
        ...
        "git_worktree_clean": not bool(subprocess.check_output(["git", "status", "--short"], text=True).strip()),
        "tracked_runtime_source_clean": subprocess.run(["git", "diff", "--quiet", "HEAD", "--", "agent", "backend/app"], check=False).returncode == 0,
        "source_snapshot_head": "965dcacc5c189f4a9808b063e32085d89e636803",
