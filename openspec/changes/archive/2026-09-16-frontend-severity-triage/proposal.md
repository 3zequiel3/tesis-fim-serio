## Current authority and historical scope

This active change now follows the canonical breaking bulk contract: approve uses `{event_ids:[...]}` and reject uses `{event_ids:[...], action:"restore"|"quarantine"}`. There is no `items` alias; legacy bodies receive 422. Client versions, confirmation flags, per-item actions and bulk `baseline_absent` are not part of the wire. The defect transcript below preserves the two incompatible `items[]` shapes observed before this superseding migration; those examples are historical evidence, not normative API definitions.
...
### Defecto 2 — diagnóstico histórico del rechazo en lote que devolvía 422

Antes de la migración, `frontend/src/api/actions.ts:64-66` publicaba `{ items: [{event_id, version}], action }`, con la acción **al nivel superior**. El backend de ese momento la exigía **por ítem**: `BulkRejectItem` declaraba `action: RejectAction` obligatoria dentro de cada elemento de `items`. Pydantic ignoraba la clave sobrante de arriba y fallaba por la que faltaba abajo.

Verificado contra la API viva antes de la migración canónica (registro histórico):
...
En ese estado histórico, el operador veía `toast.error('Error al rechazar en lote')` y nada más: el manejador descartaba el status y el cuerpo del error, por lo que el 422 estructural se presentaba igual que una caída de red. El tipo `BulkRejectItem` del frontend tampoco declaraba el campo. La mitad de US-25 era código muerto.

### Por qué sobrevivió históricamente: había dos suites verdes con creencias opuestas

El test histórico `test_bulk_reject_uses_items_contract_with_per_item_action` pasaba sobre la variante por ítem. Del otro lado, el frontend mandaba la acción arriba y su suite también pasaba. **Las dos suites verificaban la creencia de su propio lado y ninguna observaba la del otro.** El contrato tenía dos implementaciones y cero aserciones compartidas; el 422 solo era observable en el punto de encuentro que ningún test ejecutaba.

Históricamente, la causa raíz estaba un nivel más arriba: **la especificación del proyecto también estaba equivocada.** La versión vigente entonces prescribía `{items:[{event_id, version}], action}` y el frontend no se desvió de ella: la cumplió. Esa formulación queda como evidencia del defecto, no como autoridad actual; la spec base ahora fija el contrato canónico `event_ids`.
...
- **El rechazo en lote usa el wire canónico.** `bulkReject(eventIds, action)` emite `{event_ids:[...], action}` y `bulkApprove(eventIds)` emite `{event_ids:[...]}`. El backend carga los eventos, captura sus versiones actuales y conserva el conflicto optimista por carrera. No se mantienen tipos por ítem ni alias legacy.
...
- **Cambios de backend ajenos al contrato bulk.** La migración canónica sí modifica schema, router y servicio de acciones; no altera persistencia de severidad, filtros ni orden del listado.
...
- `frontend-events`: el contenido mínimo de la fila pasa a incluir severidad y contexto de proceso; el listado gana filtro por severidad sincronizado en URL; y el bulk wire queda fijado a approve `{event_ids:[...]}` y reject `{event_ids:[...], action}` como migración breaking, sin alias `items`.
...
**Frontend** — `components/ui/EventsTable.tsx` (columna y banda de severidad, contexto de proceso), `pages/Events.tsx` (control de filtro por severidad), `api/events.ts` (`EventFilters.severity` + serialización repetible), `utils/eventFilters.ts` (parse/serialize de `severity`), `pages/Dashboard.tsx` (tarjetas navegables, léxico canónico), `api/actions.ts`, `components/ui/BulkActionBar.tsx` y `hooks/useEventActions.ts` (wire canónico `event_ids` y acción compartida), más el helper `utils/severity.ts`.

**Backend** — `modules/actions/schemas.py`, `router.py` y `service.py` adoptan el wire canónico, cargan eventos por ID y conservan el conflicto optimista. Los tests compartidos aceptan `event_ids` y exigen 422 para `items`.
...
**Reglas cubiertas**: RN-128/D34 (severidad persistida y filtrable — se consume del lado de la consola lo que la decisión habilitó), RN-129/D35 (el `status` derivado es el tipo de acción — se aplica su lectura, sin alterarla), C1/RN-71 (léxico canónico — se corrige la divergencia del dashboard). **Decisiones aplicadas**: D34, D35, D3 (sin migración de base de datos; con migración breaking del API bulk). **Ninguna decisión nueva de appendix**, y la afirmación es verificable punto por punto: la severidad en la fila la pide US-06:151; el filtro por severidad lo habilita D34 explícitamente; el proceso causante lo pide US-06:151 y ya viaja en el payload; el `event_ids[]` de US-25 es el contrato canónico vigente y el anterior `items[]` quedó expresamente supersedido; y el léxico del dashboard es aplicación literal de C1. Lo único genuinamente nuevo —la codificación visual de la severidad y la forma del fixture de contrato— son decisiones de diseño, no de política, y viven en `design.md`.
