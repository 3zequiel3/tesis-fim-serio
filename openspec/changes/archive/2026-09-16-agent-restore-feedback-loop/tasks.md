...
- [x] 9.4 Commit con conventional commits, sin `Co-Authored-By`. El cuerpo debe mencionar los números del incidente (497 eventos / 3240 encolados) y por qué la suite de 417 tests no lo vio: es la parte que se pierde si solo queda el diff.
  - Cumplida por el commit `64204d6` (`fix(agent): corta el bucle infinito de auto_restore`): su cuerpo registra los 497 eventos en 4,7 minutos y los 3240 encolados, la causa (`os.replace` entrega `FAN_MOVED_TO` sobre el path final, que el filtro de `.fim_restore_tmp` no descarta y la rama `file_created` emite sin comparar contra el baseline), y por qué la suite no lo vio: los tests unitarios mockeaban el filesystem, el unit tenía `ProtectSystem=strict` sin `ReadWritePaths` —así que la restauración fallaba con EROFS y el camino nunca se ejecutaba— y las reglas del laboratorio del Capítulo 5 son `alert_only` a propósito. Sin `Co-Authored-By` ni atribución a IA.
