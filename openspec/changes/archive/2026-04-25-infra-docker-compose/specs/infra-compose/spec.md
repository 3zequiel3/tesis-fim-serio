...
El sistema SHALL incluir un `.env.example` en la raíz que documente, con un comentario explicativo, cada variable que el compose lee: `DB_PASSWORD`, `JWT_SECRET_CURRENT`, `JWT_SECRET_PREVIOUS`, `ADMIN_USERNAME`, `ADMIN_PASSWORD`, `CA_CERT_PATH`, `CA_KEY_PATH`. El archivo `.env` real SHALL NO commitearse — el `.gitignore` SHALL incluirlo.
