...
- [x] 6.2 **Revisar si la tesis cita estas specs.** **NO las cita** — cero menciones a `openspec` en el PDF. La preocupación era infundada. **Pero apareció otra cosa**: el PDF afirma `pyfanotify` en cuatro lugares —agradecimientos, cuerpo con justificación de diseño, tabla de stack y **cita bibliográfica formal**— y el agente no lo usa. Además la tabla dice Python 3.12 y el Dockerfile fija 3.13. Registrado en `thesis-document-findings.md` con qué corregir en la fuente. **El PDF no se modifica** — es el documento compilado.

- [x] 6.9 **Completar la limpieza de `pyfanotify` en `arquitectura_stack.md`.** La corrección previa fue incompleta: arregló 5 lugares y quedaban **10**, incluidos el diagrama ASCII, la justificación arquitectónica de la sección de decisiones y la tabla comparativa `inotify` vs `fanotify`. Las tres menciones que quedan son las que **explican** el abandono, no las que lo contradicen.
- [x] 6.3 y 6.4 **Regla de proceso contra la recurrencia**, incorporada a `CLAUDE.md`: correr `scripts/check_spec_integrity.py` **antes y después** de cada `openspec archive`, y prohibición explícita de escribir directorios de archive a mano. Es la causa raíz real — el CLI 1.4.1 archiva correctamente; el daño histórico incluye un archive escrito a mano (`5355465`) que ningún arreglo del CLI habría evitado.
