...
- [ ] 7.4 (reabierta — la corrida real contra el VPS mostró `ticket=[REDACTED]` en nginx pero el ticket y el JWT en claro en el backend; reverificar tras el fix de 7.10) nginx: con el stack levantado, `docker compose exec frontend nginx -t` en `CONSOLE_TLS_MODE=off` y en `self_signed`; hacer `curl -k '<consola>/api/alerts/stream?ticket=probe-ticket-123'` y confirmar que `docker compose logs frontend backend` contiene `ticket=[REDACTED]` y no contiene `probe-ticket-123`
