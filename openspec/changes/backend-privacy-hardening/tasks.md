...
- [x] 7.4 (reabierta y reverificada en el VPS el 2026-09-16 tras el fix de 7.10: la primera corrida mostró `ticket=[REDACTED]` en nginx pero el ticket y el JWT en claro en el backend; tras el fix ambos salen redactados y el access log del backend vuelve a emitirse como JSON con `"logger": "uvicorn.access"`) nginx: con el stack levantado, `docker compose exec frontend nginx -t` en `CONSOLE_TLS_MODE=off` y en `self_signed`; hacer `curl -k '<consola>/api/alerts/stream?ticket=probe-ticket-123'` y confirmar que `docker compose logs frontend backend` contiene `ticket=[REDACTED]` y no contiene `probe-ticket-123`
- [x] 7.5 (verificada en el VPS el 2026-09-16: `POST /alerts/stream-ticket` lleva el JWT en `Authorization` y devuelve `{"ticket": …, "expires_in": 30}`; el `GET /api/alerts/stream?ticket=…` que queda abierto no lleva `token=` en la URL ni headers `Authorization` o `Cookie`) Verificar que ningún request a `/alerts/stream` lleva JWT: `rg -n "stream\?token|token=\\$\{" frontend/src` sin resultados y, en la consola desplegada, la pestaña de red muestra sólo `?ticket=` (y `last_event_id` tras un corte)
- [x] 7.6 (verificada en el VPS el 2026-09-16: tras `docker compose restart backend` la consola pidió un ticket nuevo —valor distinto al anterior— y reabrió el stream; el evento #13 sobre `/srv/fim-watch/prueba-76.txt`, escrito durante el corte, apareció en el listado sin recargar la página, con la demora esperada del backoff más el arranque del backend) Prueba manual de reconexión: con la consola abierta, reiniciar el contenedor `backend`, crear una alerta durante el corte y confirmar que al reconectar llega el toast (si ya había alertas previas) y que listados y banner se actualizan
...
- [x] 7.10 Pasar `log_config=None` a los dos `uvicorn.Config` de `app/core/pki.py` (mTLS 8443 y bootstrap 8444): `uvicorn.Config.__init__` llama a su propio `configure_logging()`, que con el `log_config` por defecto re-corre `dictConfig` y reemplaza los handlers que `app.core.logging.configure_logging()` instaló en `uvicorn`, `uvicorn.error` y `uvicorn.access`. Como esos servidores se construyen dentro del lifespan, el access log quedaba desenganchado de `sanitize_secrets` + `redact_query_credentials` para el resto de la vida del proceso. Tests de regresión en `tests/test_logging_sanitize.py`: uno afirma el `log_config=None` en ambas construcciones, el otro emite una línea `uvicorn.access` después de construir un servidor real y verifica que el ticket sale redactado
