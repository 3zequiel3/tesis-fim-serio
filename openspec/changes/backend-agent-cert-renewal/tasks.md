...
- [ ] 5.5.1 Mantener `CERT_REQUIRED` y TLS 1.3 mínimo en el listener.
- [ ] 5.5.2 Test real: certificado ausente, no confiable o vencido falla durante el handshake.
- [ ] 5.5.3 El loop del agente no intenta renovar un certificado ya vencido y registra que requiere recuperación administrativa.
- [ ] 5.5.4 Documentar que la recuperación administrativa debe preservar el `master_secret` o realizar un re-baseline explícito.
