...
- [x] 13.1 (verificado en el host del agente el 2026-09-16: `CapEff = CapAmb = 000000000020000f`, que decodifica en los bits 0, 1, 2, 3 y 21 — `CAP_CHOWN`, `CAP_DAC_OVERRIDE`, `CAP_DAC_READ_SEARCH`, `CAP_FOWNER` y `CAP_SYS_ADMIN` —, exactamente las cinco declaradas en `agent/deploy/fim-agent.service:32`. Que `CapAmb` tenga la misma máscara es lo que prueba que son ambient y no sólo permitidas: en `CapPrm` sin `CapAmb` el proceso las perdería al arrancar y fanotify fallaría) Las cinco ambient capabilities están efectivamente otorgadas: `grep Cap /proc/$(systemctl show -p MainPID --value fim-agent)/status` + `capsh --decode=<valor>`.
- [x] 13.2 (verificado en el host del agente el 2026-09-16 con dos corridas de `systemd-run --property=ProtectSystem=strict` que consultan `os.statvfs` como el preflight. Con `ReadWritePaths=/var/lib/fim-agent` solamente: `/var/lib/fim-agent` escribible, `/srv/fim-watch` y `/etc` en `ST_RDONLY`. Agregando `ReadWritePaths=/srv/fim-watch`: ese mismo path pasa a escribible y `/etc` sigue en `ST_RDONLY`. Mismo path y misma máquina con una sola variable cambiada, así que el resultado aísla el efecto del drop-in y no el de `ProtectSystem` en general. El drop-in real lo genera `python -m agent.deployment` en `/etc/systemd/system/fim-agent.service.d/10-watchpaths.conf` con `/etc/fim-agent` y `/srv/fim-watch`) `statvfs` reporta `ST_RDONLY` para un path fuera de `ReadWritePaths` **dentro del namespace del servicio**, y no lo reporta para uno cubierto por el drop-in. Es la asunción central de D-3 del design; verificarla con `systemd-run --unit=probe --property=... ` o con un script corrido bajo el mismo unit.
- [x] 13.3 (verificado en el host del agente el 2026-09-16 sobre `/srv/fim-watch/a4.txt`, con la regla `/srv/fim-watch/*` → `auto_restore` sincronizada como `ruleset_version=3`. Antes: `root:root 644`. Se cambió dueño y contenido a la vez — `chown ezequiel:ezequiel` más un append — porque un `chown` solo podría no disparar el evento. Después: `root:root 644` y la línea agregada ya no está en el archivo. Atribuido al agente por el log: `decision.evaluated action=auto_restore action_failed=false` sobre ese path) `os.chown` restaura uid/gid reales: modificar un archivo root-owned bajo un `watch_path`, dejar que el `auto_restore` corra, y verificar con `stat` que dueño y grupo volvieron a los del baseline.
- [x] 13.4 (verificado en el host del agente el 2026-09-16 sobre `/srv/fim-watch/setuid-test`, copia de `/bin/true` en modo `4755 root:root`, baselineada por el `init_scan` de un reinicio deliberado. Adulteración doble: `chmod 0755` —quitando el bit— más un append de contenido. Después de la restauración: `root:root 4755`, con `decision.evaluated action=auto_restore action_failed=false`. Es la comprobación del orden `os.fchown` antes de `os.fchmod` (`agent/decision.py:274-275`): invertirlo devolvería el contenido, reportaría éxito y dejaría el binario sin setuid. Nota de lectura del log: hay un primer evento con `action_failed=true` desde el pid anterior, que corresponde a la **creación** del archivo cuando todavía no existía baseline para él — artefacto del montaje de la prueba, no del mecanismo) El bit setuid sobrevive: mismo ejercicio sobre un binario setuid de prueba, verificando que el bit sigue puesto tras la restauración.
- [x] 13.5 (verificada el 2026-09-16 en dos mitades. **Instalación e idempotencia**: `agent/tests/integration/test_install_sh_container.py`, 11 tests en 2:23, que corre el `install.sh` real como root dentro de un contenedor descartable con un shim de `systemctl` — cubre instalación limpia con unit, config, drop-in y servicio habilitado, segunda corrida byte-idéntica, `--reconfigure`, reemplazo de código con borrado de archivos obsoletos, rechazo del flag de secreto, paths relativos y versión de Python incorrecta. **Lo que necesita systemd real**, medido en el host del agente: `systemd-analyze security fim-agent.service` → **6.7 MEDIUM**, y arranque en frío sin `FIM_BOOTSTRAP_SECRET` mediante un unit transitorio con la misma política de reinicio → `ExecMainStatus=78`, `Result=exit-code`, **`NRestarts=0`** y `status=78/CONFIG` en el journal, o sea sin loop de reinicios, que es lo que garantiza `RestartPreventExitStatus=78`. Salvedad honesta: el score "antes" no quedó registrado en ningún artefacto previo, así que 6.7 es el valor actual y no hay comparación reconstruible. Hallazgo menor abierto: el único ✗ del análisis es `UMask=` — los archivos creados sin modo explícito quedan world-readable; el agente fija 0700 en directorios y 0600 en certificados, así que la exposición real es acotada, pero `UMask=0077` en el unit lo cerraría) `install.sh` de punta a punta sobre un host limpio: unit + config + drop-in + servicio habilitado; segunda corrida para confirmar idempotencia; `systemd-analyze security fim-agent.service` y registrar el score antes/después; y el arranque en frío sin secreto de bootstrap terminando en `failed` con exit 78 y **sin** loop de reinicios.
...
- [x] 15.4 (cumplida por los slices ya en la historia, todos conventional commits: `2a40b7a` preflight de escritura y generador del drop-in de systemd, `af25c50` capabilities de remediación y propiedad root del código, `656b101` chequeo de escribibilidad con ids efectivos, `7ae19d7` parametrización de `install.sh` con trust anchor verificado, y `b644d3a` endurecimiento de `install.sh` para hosts reales. Un barrido de toda la historia por `Co-Authored-By` y por `Generated with` no devuelve ninguna coincidencia) Commits convencionales, sin atribución a IA. Slices sugeridos: (a) funciones puras del agente + tests puros, (b) unit + drop-in + `install.sh` + propiedad, (c) restauración con metadata + vocabulario de razones + tests de filesystem, (d) preflight + heartbeat, (e) backend (columnas, migraciones, ingesta, consumer), (f) frontend, (g) docs canónicas + roadmap.
